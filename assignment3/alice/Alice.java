// Author: Shradheya Thakre (A0161476B)
...
class Alice { // Alice is a TCP  client
    ...
    // Send session key to Bob
    public void sendSessionKey(Socket clientSocket) {
    	try {
    		SealedObject sealedObject = crypto.getSessionKey();
    		toBob = new ObjectOutputStream(clientSocket.getOutputStream());
    		toBob.writeObject(sealedObject);
    	} catch(Exception e) {
    		e.printStackTrace();
    	}
    }
    ...
    public void receiveMessages(Socket clientSocket) throws IOException {
    	FileWriter fileWriter = new FileWriter(MESSAGE_FILE);
    	try{
    		fromBob = new ObjectInputStream(clientSocket.getInputStream());
    		SealedObject encodedMessageObject = (SealedObject)this.fromBob.readObject();
    		fileWriter.write(crypto.decryptMessage(encodedMessageObject) + "\n");
    		//System.out.println(crypto.decryptMessage(encodedMessageObject));
    		while(encodedMessageObject != null) {
    			encodedMessageObject = (SealedObject)this.fromBob.readObject();
    			fileWriter.write(crypto.decryptMessage(encodedMessageObject) + "\n");
    			//System.out.println(crypto.decryptMessage(encodedMessageObject));
    		}
    	} catch(Exception e) {
    ...
    class Crypto {
    	...
    	public SealedObject getSessionKey() throws Exception {
